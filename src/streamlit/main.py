...
import ast
from utils import get_naive_globe, get_genetic_globe
...
with st.sidebar:
    with st.form("input_form"):
        ...
        st.write('For genetic algorithm: ')
        population_size = st.slider('Population size', num_locs, 300, 150)
        generations = st.slider('Generations', 1, 1000, 200)
    ...
    if st.button('Get locations and generate flight path!'):
        try:
            if num_locs is not None:
                ...
                session.call("routes.genetic_path_finder", table_name, population_size, generations, mutation_rate)
                data_ready = True 
        finally:
            st.session_state.table = table_name
...
if st.session_state.clicked:
    ...
    genetic_solution = df[df['METHOD'] == 'genetic']
    genetic_path = ast.literal_eval(genetic_solution['PATH'].iloc[0])
    genetic_path_distance = genetic_solution['DISTANCE'].iloc[0]

    naive_fig = get_naive_globe(naive_path)
    globe_fig = get_genetic_globe(genetic_path)
    ...
    with col2:
        st.header('Genetic Solution')
        st.write(f'Total travel distance (km): {int(genetic_path_distance)}')
